...
class BankStatement(BaseModel):
    ...
    periodo_desde: str | None = Field(
        None, description="Statement period start, format YYYY-MM-DD"
    )
    periodo_hasta: str | None = Field(
        None, description="Statement period end, format YYYY-MM-DD"
    )
    moneda: str = Field("EUR", description="Currency code")
    lineas: list[BankLine] = Field(
        default_factory=list, description="Transaction lines"
    )
