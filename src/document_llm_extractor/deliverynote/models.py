...
class DeliveryNoteReport(BaseModel):
    """Complete schema for delivery note extraction.
    ...
    Attributes:
        numero_deliverynote: Unique delivery note number (required).
        fecha_deliverynote: Date of document in YYYY-MM-DD format (required).
        ...
        total_deliverynote: Final total amount (required).
    """

    model_config = ConfigDict(extra="forbid")

    # Document data
    numero_deliverynote: str = Field(..., description="Unique delivery note number")
    fecha_deliverynote: str = Field(
        ..., description="Date of document, format YYYY-MM-DD"
    )
    categoria_gasto: str | None = Field(None, description="Expense category")
    fecha_registro: str | None = Field(
        None, description="Registration date, format YYYY-MM-DD"
    )
    ...
    porcentaje_retencion: float | None = Field(
        None, description="Withholding percentage"
    )
    importe_retencion: float | None = Field(None, description="Withholding amount")
    total_deliverynote: float = Field(..., description="Final total amount")
