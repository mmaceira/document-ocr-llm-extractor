...
def redact_sensitive_info(text: str) -> str:
    ...
    text = re.sub(
        r"\b[A-Z]{2}\d{2}[A-Z0-9]{11,30}\b", "••REDACTED-IBAN••", text, flags=re.I
    )
    # Redact Spanish tax IDs
    return re.sub(
        r"\b(\d{8}[A-Z]|[A-Z]\d{8})\b", "••REDACTED-TAXID••", text, flags=re.I
    )


def map_fields_to_ocr_items(
    report: Any, ocr_items: list[dict[str, Any]]
) -> dict[int, str]:
